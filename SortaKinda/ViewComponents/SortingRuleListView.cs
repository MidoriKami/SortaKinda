...
public class SortingRuleListView(SortController sortController, List<SortingRule> rules) {
    ...
    private void AddNewRuleButton() {
        if (ImGuiTweaks.IconButtonWithSize(Service.PluginInterface.UiBuilder.IconFontFixedWidthHandle, FontAwesomeIcon.Plus, "AddNewRuleButton", ImGui.GetContentRegionAvail() with { Y = 23.0f * ImGuiHelpers.GlobalScale })) {
    ...
    private void DrawDeleteButton(int index) {
        ...
        using (var _ = ImRaii.Disabled(!(ImGui.GetIO().KeyShift && ImGui.GetIO().KeyCtrl))) {
            if (ImGuiTweaks.IconButtonWithSize(Service.PluginInterface.UiBuilder.IconFontFixedWidthHandle, FontAwesomeIcon.Trash, index.ToString(), buttonSize)) {
